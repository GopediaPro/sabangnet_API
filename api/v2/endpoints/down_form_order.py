...
async def db_to_excel_url(
    ...
    try:
        ...
        # 날짜 범위로 데이터 조회
        down_form_orders = await down_form_order_read_service.get_down_form_orders_by_date_range(
            date_from=ord_st_date,
            date_to=ord_ed_date
        )
        ...
        try:
            ...
            logger.info(f"[db_to_excel_url] 완료 - URL: {file_url}, 레코드 수: {len(down_form_orders)}")
...
async def get_excel_to_db(
    ...
    """
    try:
        logger.info(f"[get_excel_to_db] 시작 - 파일: {file.filename}, 요청자: {request_id}")
        ...
        # Upsert 처리
        inserted_count, updated_count = await down_form_order_update_service.bulk_upsert_from_excel(dto_items)
        
        logger.info(f"[get_excel_to_db] 완료 - 삽입: {inserted_count}, 업데이트: {updated_count}")
