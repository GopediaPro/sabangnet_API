...
def run_ect_site_macro(file):
    macro = ERPEtcSiteMacro(file)
    return macro.etc_site_macro_run()


def run_zigzag_macro(file):
    macro = ERPZigzagMacro(file)
    return macro.zigzag_erp_macro_run()


def run_ali_macro(file):
    macro = ERPAliMacro(file)
    return macro.ali_erp_macro_run()


def run_brandi_macro(file):
    macro = ERPBrandiMacro(file)
    return macro.brandi_erp_macro_run()


def run_gmarket_auction_macro(file_path):
    macro = ERPGmaAucMacro(file_path)
...
def preprocess_order_data(order_data, template_code):
    ...
    # 숫자 데이터 변환
    numeric_fields = ['quantity', 'product_price',
                      'option_price', 'delv_cost', 'total_amount']
    ...
    if is_jeju_address(processed['address']):
        processed['item_name'] = add_jeju_notice(
            processed['item_name'], processed['address'])
        processed['is_jeju'] = True
    else:
        processed['is_jeju'] = False

    # 다중 수량 체크
    processed['is_multiple_quantity'] = check_multiple_quantities(
        processed['item_name'])
...
def process_orders_for_db(orders_list, template_code):
    ...
    # 1. 개별 주문 전처리
    processed_orders = [preprocess_order_data(
        order, template_code) for order in orders_list]
    ...
    if '브랜디' in processed_orders['site_name']:
        ...
        processed_orders = [merge_order_group(
...
def merge_order_group(order_group):
    ...
    # G열(수량) 합산
    total_quantity = sum(convert_to_number(order.get('quantity', 0))
                         for order in order_group)
