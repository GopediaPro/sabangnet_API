...
class EcountAuthService:
    ...
    async def get_zone_info(self, com_code: str, is_test: bool = True) -> Optional[ZoneResponse]:
        ...
        data, status, error = await aiohttp_post(url, zone_request.model_dump(), timeout=self.session_timeout, logger=logger)
    ...
    async def login(self, auth_info: EcountAuthInfo, is_test: bool = True) -> Optional[LoginResponse]:
        ...
        data, status, error = await aiohttp_post(url, login_request.model_dump(), timeout=self.session_timeout, logger=logger)
        if status == 200 and data:
    ...
    async def authenticate(self, com_code: str, user_id: str, api_cert_key: str, is_test: bool = True) -> Tuple[Optional[str], Optional[EcountAuthInfo]]:
        """
        완전한 인증 프로세스를 수행합니다.
    ...
    async def authenticate_with_env(self, is_test: bool = True) -> Tuple[Optional[str], Optional[EcountAuthInfo]]:
        """
        ...
        if not all([com_code, user_id, api_cert_key]):
            ...
            logger.error(f"ECOUNT_COM_CODE: {com_code}, ECOUNT_USER_ID: {user_id}, ECOUNT_API: {'설정됨' if api_cert_key else '설정되지 않음'}")
            return None, None
    ...
    async def validate_session(self, session_id: str, auth_info: EcountAuthInfo, is_test: bool = True) -> bool:
        """세션의 유효성을 검증합니다."""
        login_response = await self.login(auth_info, is_test)
        if not login_response or not login_response.Data:
            logger.error("로그인 실패")
...
class EcountAuthManager:
    ...
    async def get_authenticated_info(self, com_code: str, user_id: str, api_cert_key: str, is_test: bool = True) -> Optional[EcountAuthInfo]:
    ...
    async def get_authenticated_info_from_env(self, is_test: bool = True) -> Optional[EcountAuthInfo]:
    ...
    def clear_cache(self, com_code: str = None, user_id: str = None, is_test: bool = None):
        """캐시를 클리어합니다."""
