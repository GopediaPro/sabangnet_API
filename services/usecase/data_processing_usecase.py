...
class DataProcessingUsecase(DataProcessingFunctions):
    ...
    async def save_down_form_orders_from_receive_orders(
        ...
        if config['is_aggregated']: 
            ...
            processed_data = await self.process_aggregated_data(raw_data, config)
        else:
            logger.info("Simple template detected. Processing simple data.")
            processed_data = await self.process_simple_data(raw_data, config)
        logger.info(f"Data processed. processed_data_count={len(processed_data)}. Sample: {processed_data[:3]}")
        
        # 3. down_form_orders에 저장
        saved_count = await self.save_to_down_form_orders(processed_data, template_code)
    ...
    async def process_excel_to_down_form_orders(self, df, template_code: str) -> int:
        ...
        logger.info(f"Loaded template config: {config}")
        # 2. 엑셀 데이터 변환
        raw_data = await self.process_excel_data(df, config)
        # 3. DB 저장
        saved_count = await self.save_to_down_form_orders(raw_data, template_code)
