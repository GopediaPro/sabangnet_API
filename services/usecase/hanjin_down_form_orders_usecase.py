...
class HanjinDownFormOrdersUsecase(HanjinWblsAdapter):
    ...
    async def insert_invoice_no_to_down_form_orders(self, limit: int = 100) -> list[BaseDownFormOrder]:
        ################################### step1. invoice_no가 없는 주문 데이터 조회 ###################################
        ...
        ################################### step2. hanjin_printwbls 테이블에 기초 데이터 입력 ###########################
        ...
        ################################### step6. 응답 데이터로 hanjin_printwbls 완성 #################################
        updated_records: list[HanjinPrintwbls] = []
        failure_records: list[HanjinPrintwbls] = []
        if api_response.address_list:
            for i, address_result in enumerate(api_response.address_list):
                if i < len(base_records):
                    ...
                    response_data: dict[str, Any] = address_result.model_dump()
                    if 'result_code' in response_data and "ERROR" in response_data['result_code']:
                        record.result_code = response_data['result_code']
                        record.result_message = response_data['result_message']
                        logger.info(f"hanjin_printwbls 레코드 업데이트 실패: {response_data['result_code']} {response_data['result_message']}")
        ...
        logger.info(f"hanjin_printwbls {len(updated_records)}건 API 처리 및 업데이트 완료, 실패: {len(failure_records)}건")

        ################################### step7. 응답 데이터로 down_form_orders invoice no 업데이트 ###################
