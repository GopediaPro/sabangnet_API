"""
브랜디 합포장 자동화 스크립트
VBA <브랜디_합포장_자동화.bas> 의 1-14단계를 Python(OpenPyXL) 로 이식한 버전.
"""
...
def brandy_merge_packaging(file_path: str) -> str:
    ...
    for i in range(2, last_row + 1):
        u_val = _to_number(ws[f"U{i}"].value)
        v_val = _to_number(ws[f"V{i}"].value)
        ws[f"D{i}"].value = u_val + v_val

    # ── 3단계: (이미 값) ────────────────────────

    # ── 4~6단계: 그룹핑·금액합산·모델명 병합·중복 삭제
    ...
    for idx_list in groups.values():
        ...
        total_amount = sum(_to_number(ws[f"D{r}"].value) for r in idx_list)
    ...
    # ── 9단계: 전화번호 포맷 ────────────────────
    for i in range(2, last_row + 1):
        ws[f"H{i}"].value = _format_phone(ws[f"H{i}"].value)
        ws[f"I{i}"].value = _format_phone(ws[f"I{i}"].value)
    ...
    rows.sort(key=lambda x: _to_number(x[3]))  # D열(4번째) 기준
