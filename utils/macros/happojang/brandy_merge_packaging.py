...
def brandy_merge_packaging(file_path: str) -> str:
    ...
    for i in range(2, last_row + 1):
        u_val = to_number(ws[f"P{i}"].value)
        v_val = to_number(ws[f"V{i}"].value)
    ...
    for idx_list in groups.values():
        ...
        total_amount = sum(to_number(ws[f"D{r}"].value) for r in idx_list)
    ...
    rows.sort(key=lambda x: to_number(x[3]))  # D열(4번째) 기준
