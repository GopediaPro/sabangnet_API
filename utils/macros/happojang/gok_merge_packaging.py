...
HDR_FILL    = PatternFill(start_color="00B050", end_color="00B050", fill_type="solid")
...
def gok_merge_packaging(file_path: str) -> str:
    ...
    for r in range(2, last_row + 1):
        ws[f"D{r}"].value = (
            _to_num(ws[f"O{r}"].value) +
            _to_num(ws[f"P{r}"].value) +
            _to_num(ws[f"V{r}"].value)
        )

    # 5. F열 문자열 정리
    for r in range(2, last_row + 1):
        ws[f"F{r}"].value = _clean_f_text(ws[f"F{r}"].value)
    ...
    for r in range(2, last_row + 1):
        acc = _bracket_text(ws[f"B{r}"].value)
    ...
    # 저장
    output_path = str(Path(file_path).with_name(Path(file_path).stem + "_processed.xlsx"))
    wb.save(output_path)
    wb.close()
