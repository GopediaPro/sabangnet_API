...
def average_duplicate_cart_address_amounts_df(df):
    """
    DataFrame에서 장바구니번호와 수취인주소가 같은 값들의 금액을 평균내는 메서드
    ...
    for idx, row in df.iterrows():
        cart_number = str(row.get('장바구니번호', '')).strip()
        address = str(row.get('수취인주소', '')).strip()
        amount = row.get('금액', 0)
    ...
    for key, group in duplicate_groups.items():
        if len(group) >= 2:
            ...
            # 모든 행에 평균값 적용
            for item in group:
                df.loc[item['idx'], '금액'] = average_amount
