...
class GmarketAuctionERP:
        ...
        if len(self.df.columns) > 2:
            ...
            for row_idx, row_data in enumerate(self.df.itertuples(index=False), start=2):
                for col_idx, value in enumerate(row_data, start=1):
                    self.ws.cell(row=row_idx, column=col_idx, value=value if value else "")
    ...
    def _step_5(self, ws=None):
        """
        [5단계] Q, P, M, W 숫자 변환 및 오른쪽 정렬
        """
        ...
        print("5단계: Q, P, M, W 숫자 변환 및 오른쪽 정렬 완료")
    ...
    def _step_7(self, ws):
        ...
        self.ex.highlight_column(
            col='F', light_color=self.light_blue_fill, ws=ws, start_row=2, last_row=self.last_row)
