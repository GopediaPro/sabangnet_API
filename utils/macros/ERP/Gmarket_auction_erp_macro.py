...
def gmarket_auction_erp_1_to_8(file_path):
    ...
    #     ws[f'D{row}'].value = '=INDIRECT("O" & ROW()) + INDIRECT("P" & ROW()) + INDIRECT("V" & ROW())'
    ...
    for row in range(2, q_last_row + 1):
        basket_no = str(ws[f'Q{row}'].value).strip(
        ) if ws[f'Q{row}'].value else ""
    ...
    for row in range(2, e_last_row + 1):
        ...
        if e_value and str(e_value).replace('.', '').replace('-', '').isdigit():
            ws[f'E{row}'].value = float(e_value) if '.' in str(
                e_value) else int(float(e_value))
...
def run_steps_9_to_11(file_path):
    ...
    if len(df.columns) > 2:
        df = df.sort_values(
            by=[df.columns[2], df.columns[1]], na_position='last')
    ...
    for row in range(1, ws_source.max_row + 1):
        ws_source[f'D{row}'].number_format = 'General'
        ws_source[f'P{row}'].number_format = 'General'

    # --- 9-4단계: 정렬 끝난 뒤 D열 수식 삽입 ---------------------------
    for row in range(2, last_row + 1):       # 2행부터 데이터 길이만큼
        ws_source[f'D{row}'].value = f"=O{row}+P{row}+V{row}"
        print(f"D{row} 수식 삽입: =O{row}+P{row}+V{row}")

    output_path = file_path.replace('.xlsx', '_매크로_완료.xlsx')
    workbook.save(output_path)
    print(f"9단계 처리 완료! 파일 저장: {output_path}")
    ...
    for idx, row_data in df.iterrows():
        ...
        # B열(사이트 정보)에서 계정명 추출
        site_value = str(row_data.iloc[1]) if pd.notna(
            row_data.iloc[1]) else ""
        account_name = ""
